  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-100 to-brown-200 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <span className="absolute bottom-10 left-10 text-4xl opacity-20">
          🐾
        </span>
        <span className="absolute bottom-20 right-20 text-3xl opacity-15">
          🐕
        </span>
        <span className="absolute bottom-20 left-20 text-4xl opacity-20">
          🐾
        </span>
        <span className="absolute bottom-10 right-10 text-3xl opacity-15">
          🐱
        </span>
        <span className="absolute bottom-1/2 left-1/4 text-5xl opacity-10">
          🐾
        </span>
        <span className="absolute bottom-1/3 right-1/3 text-4xl opacity-20">
          🐾
        </span>
        <span className="absolute bottom-2/3 left-1/3 text-3xl opacity-15">
          🐶
        </span>
        <span className="absolute bottom-3/4 right-1/4 text-4xl opacity-20">
          🐾
        </span>
      </div>
      <Navbar fluid rounded className="sticky top-0 z-50">
        ...
        <NavbarCollapse>
          ...
          <Dropdown label="Quản lý" inline>
            <DropdownItem onClick={() => navigate("/appointment-form")}>
              Đặt lịch (online)
            </DropdownItem>
            <DropdownItem onClick={() => navigate("/appointment-list")}>
              Xem lịch khám
            </DropdownItem>
            <DropdownItem onClick={() => navigate("/walk-in")}>
              Tiếp khách walk-in
            </DropdownItem>
            <DropdownItem onClick={() => navigate("/exam-form")}>
              Khám bệnh
            </DropdownItem>
            <DropdownItem onClick={() => navigate("/medical-history")}>
              Lịch sử khám
            </DropdownItem>
            <DropdownItem onClick={() => navigate("/sales")}>
              Bán hàng
            </DropdownItem>
            <DropdownItem onClick={() => navigate("/invoice")}>
              Hóa đơn
            </DropdownItem>
            <DropdownItem onClick={() => navigate("/inventory")}>
              Kho / Vắc-xin
            </DropdownItem>
            ...
            <DropdownItem onClick={() => navigate("/demo")}>
              Demo Data
            </DropdownItem>
          </Dropdown>
