console.log('replacing with minimal stub');
...
async function seedDemoAppointments(total = 70000) {
  ...
  while (inserted < total) {
    ...
    for (let i = 0; i < toInsert; i++) {
      const idx = inserted + i + 1;
      const petname = `SeedPet${idx}`;
      const ownername = `Owner${idx}`;
      const phone = `090${(1000000 + idx).toString().slice(-7)}`;
      const service = ['vet','grooming','vaccine'][idx % 3];
      const date = new Date(Date.now() + (idx % 365) * 86400000).toISOString();
      const time = `${(8 + (idx % 10)).toString().padStart(2,'0')}:00`;
      const notes = null;
      params.push(petname, ownername, phone, service, date, time, notes);
      const base = i * 7;
      values.push(`($${base+1},$${base+2},$${base+3},$${base+4},$${base+5},$${base+6},$${base+7})`);
    }
    const q = `INSERT INTO demo_appointments (petname, ownername, phone, service, date, time, notes) VALUES ${values.join(',')}`;
    await pool.query(q, params);
    inserted += toInsert;
    if (inserted % (batch*5) === 0) console.log('Inserted', inserted);
  }
...
async function seedDemoMedicalRecords(total = 20000) {
  ...
  while (inserted < total) {
    ...
    for (let i = 0; i < toInsert; i++) {
      const idx = inserted + i + 1;
      const pet_id = (idx % 1000) + 1;
      const exam_date = new Date(Date.now() - (idx % 1000) * 86400000).toISOString();
      const diagnosis = `Diagnosis ${idx}`;
      const notes = `Notes ${idx}`;
      params.push(pet_id, exam_date, diagnosis, notes);
      const base = i * 4;
      values.push(`($${base+1},$${base+2},$${base+3},$${base+4})`);
    }
    const q = `INSERT INTO demo_medical_records (pet_id, exam_date, diagnosis, notes) VALUES ${values.join(',')}`;
    await pool.query(q, params);
    inserted += toInsert;
    if (inserted % (batch*5) === 0) console.log('Inserted medical records', inserted);
  }
...
async function seedProductsAndInventory() {
  console.log('Ensuring sample products and inventory');
  await pool.query(`INSERT INTO product (id, name, price) VALUES ('food_seed','Seed Food',100) ON CONFLICT (id) DO NOTHING`);
  await pool.query(`INSERT INTO inventory (product_id, warehouse_id, quantity) VALUES ('food_seed', 1, 1000) ON CONFLICT DO NOTHING`);
}
...
async function seedInvoices(total = 10000) {
  ...
  while (inserted < total) {
    ...
    for (let i = 0; i < toInsert; i++) {
      ...
      const total_amount = 100000 + (idx % 1000) * 100;
      const created_at = new Date(Date.now() - (idx % 365) * 86400000).toISOString();
      params.push(total_amount, created_at);
      const base = i * 2;
      values.push(`($${base+1},$${base+2})`);
    }
    const q = `INSERT INTO demo_invoices (total_amount, created_at) VALUES ${values.join(',')}`;
    await pool.query(q, params);
    inserted += toInsert;
    if (inserted % (batch*5) === 0) console.log('Inserted invoices', inserted);
  }
